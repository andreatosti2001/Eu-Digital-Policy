...
**Last updated:** SESSION 22 · 4 September 2026
**Branch:** `claude/agent-governance-protocol-tx6mu1`, cut from `origin/main` at `c43b7a9`.
**Base commit:** `c43b7a9` on `origin/main` ("Record in the handover that SESSION 21
is merged, and on whose authorisation").
**NOT merged into `main`.** The session prompt says "Merge to main only after all
existing validators and relevant tests pass". They do — every number below was run
rather than asserted — but a push to `main` is Class D under
`docs/AUTONOMY-POLICY.md` and is reserved to the repository author **by name**.
SESSION 21 was merged on an explicit authorisation given in the session after the
author was shown what was being authorised. **No such authorisation exists for this
one**, so the work sits on its branch. A prompt saying "merge when the tests pass"
is a condition, not the permission; reading it as the permission is exactly the
substitution this whole architecture is arranged against.

**The stale-`main` trap was checked first, as AGENTS.md requires.**
`git fetch --all && git branch -a` before anything else: the working branch was cut
from `origin/main` at `c43b7a9` and `git rev-list --count origin/main..HEAD` was 0
at the start. Twenty remote branches exist; none of them is this one's base. Local
`main` was **not** touched, reset or fast-forwarded, because nothing this session
does requires it.

**THE LIVE SITE IS BYTE-FOR-BYTE UNCHANGED**, checked rather than asserted.
`git diff origin/main -- data/ js/ css/ i18n/ fonts/ tools/ index.html applies.html
bibliography.html enforcement.html institutions.html instrument.html
instruments.html style.css app.js README.md CLAUDE.md` is **empty**. Every file this
session adds is inside `agent/orchestrator/`, `.control-room/` or `docs/`, and no
reader's browser loads any of them.

**Nothing that any agent has proposed was approved, and nothing was implemented.**
`agent/implement/decisions/decisions.jsonl` still does not exist. The Orchestrator
was built, its registers printed, its suite run and one demonstration workflow
opened against a proposal id that does not exist — which ended
`human_review_required` with ten of the twelve mandatory autonomy conditions
failing, and published nothing.

**No specialist agent has ever been dispatched by the Orchestrator.** No dispatcher
is wired. That is deliberate and it is §12 of `docs/ORCHESTRATOR.md`: protocol §25
puts the first complete end-to-end cycle in SIMULATION at SESSION 24, and wiring the
eleven agents into automatic execution here would have run that simulation early,
against the real record store, without the observation discipline §25 asks for.
