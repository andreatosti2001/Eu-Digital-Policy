...
function sourceCard(ref) {
  ...
  const inst = src.publisher && IX.institution.has(src.publisher)
  ...
  return '<div class="evi-src" data-supports="' + esc(sup) + '">' +
    '<div class="evi-src-top">' +
      '<span class="evi-tier" data-tier="' + esc(src.id === 'src-brief-original' ? 'self' : src.tier) + '">' +
        esc(F.tierWord(src)) + '</span>' +
      '<span class="evi-supports" title="' + esc(F.SUPPORTS_GLOSS[ref.supports] || '') + '">' +
        esc(F.SUPPORTS_WORD[ref.supports] || sup) + '</span>' +
    '</div>' +
    '<p class="evi-cite">' + F.citeHTML(src, IX, 'note') + '</p>' +
    (inst ? '<dl class="evi-dl">' + row('Institution', esc(inst)) +
...
function renderClaim(entry) {
  ...
  const cards = (c.sources || []).map(sourceCard).join('') ||
    '<div class="evi-warn"><b>No source recorded</b>This claim has no source attached at all.</div>';
