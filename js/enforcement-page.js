...
/* the human name of each filter and of the value it is set to; the chip has
   to say "Authority: Irish DPC", not "authority: dpa-ie" */
const FILTER_LABEL = {
  instrument: 'Instrument', authority: 'Authority', action: 'Action status',
  payment: 'Payment', stage: 'Reached stage',
};
function filterValueLabel(key, value) {
  if (key === 'instrument') return (IX.instrument.get(value) || {}).short_name || value;
  if (key === 'authority') return (IX.institution.get(value) || {}).short_name || value;
  return taxLabel(IX, value) || value;
}

function activeFilters() {
  return Object.entries(filters)
    .filter(([, v]) => v)
    .map(([k, v]) => ({ key: k, label: FILTER_LABEL[k] || k, value: filterValueLabel(k, v) }));
}

function setFilter(key, value) {
  filters[key] = value || '';
  const ids = { instrument: 'f-i', authority: 'f-a', action: 'f-s', payment: 'f-p', stage: 'f-t' };
...
function render() {
  ...
  document.getElementById('enfList').innerHTML = list.length
    ...
    : emptyState('enforcement records',
        'Every record in this register is an action that was actually taken; a combination with no ' +
        'results means no such action is recorded, not that none exists.');

  renderFilterState({
    host: document.getElementById('enfState'),
    active: activeFilters(),
    count: { shown: list.length, total: RECORDS.length },
    onRemove: (k) => setFilter(k, ''),
    onClear: clearFilters,
  });
  /* the empty state carries its own clear-all, because that is where the
     reader is looking when the list is empty */
  const inline = document.querySelector('#enfList [data-clear]');
...
function focusFromHash() {
  ...
  if (Object.values(filters).some(Boolean)) clearFilters();
...
(async function boot() {
  ...
  try {
    ...
    /* a filtered view is a URL: arriving with one applies it */
    filters = readUrl(filters);
    controls();
    const stateRow = document.createElement('div');
    stateRow.className = 'filters-state';
    stateRow.id = 'enfState';
    const ctrls = document.getElementById('enfControls');
    ctrls.parentNode.insertBefore(stateRow, ctrls.nextSibling);
    render();
    focusFromHash();
    ...
    document.getElementById('enfControls').addEventListener('change', (e) => {
      ...
      setFilter(map[e.target.id], e.target.value);
    });
