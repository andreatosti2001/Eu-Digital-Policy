...
/* ================= search: the prose provider =================
   The palette itself now lives in js/palette.js and runs on every page,
   because the entity index it searches was always global while the only
   surface that could reach it was here. What is genuinely local to the
   brief is the *prose*: the thirteen Parts, and the glossary definitions
   as they are rendered in the panel. Neither exists on a tool page.

   So this file no longer owns a palette. It owns a provider: a function
   that turns a query into passage, reference and definition groups, which
   palette.js renders below the entity answers in the same list, with the
   same keyboard model and the same dialog contract. Deleting the second
   implementation is the point — there is one result row, one selection
...
window.__EU_PROSE_SEARCH__ = function(q){
  var P = window.__EU_PALETTE__ || {};
  var snip = P.snippet || function(t){ return String(t||'').slice(0,110); };
  var esc2 = P.esc || function(t){ return String(t||''); };
  var ql = String(q||'').trim().toLowerCase();
  var artMatch = String(q||'').trim().match(ART_RE);
  var groups = [];
  ...
  /* the entity index carries the glossary as first-class concepts; the
     DOM-scraped definitions are only a fallback for when it has not loaded */
  if (!window.__EU_ENTITY_SEARCH__){
    [].slice.call(document.querySelectorAll('.gloss-entry')).forEach(function(el){
      var b = el.querySelector('b'), sp = el.querySelector(':scope > span');
      var term = b ? b.textContent : '', def = sp ? sp.textContent : '';
      if (term.toLowerCase().indexOf(ql) > -1 || def.toLowerCase().indexOf(ql) > -1){
        defHits.push({ gloss: el.id.replace('gloss-',''), mark:'§', title:term,
          sub: snip(def, term.toLowerCase().indexOf(ql)>-1 ? term : q) });
      }
    });
  }
  DATA.search.forEach(function(s){
  ...
  if (refHits.length) groups.push({ kind:'reference', label:'References in the text', items: refHits.slice(0,6) });
  if (passHits.length) groups.push({ kind:'passage', label:'Passages', items: passHits.slice(0,10) });
  return groups;
};

/* the palette needs the brief's own navigation, which knows about the tree,
   the progress spine and the reveal groups — a bare scrollIntoView does not */
window.__EU_JUMP__ = jumpTo;
window.__EU_OPEN_GLOSSARY__ = function(id){
  openPanel();
  var ent = document.getElementById('gloss-' + id);
  if (ent){
    ent.scrollIntoView({block:'center'});
    ent.classList.add('lit');
    setTimeout(function(){ ent.classList.remove('lit'); }, 1600);
  }
};
...
document.addEventListener('keydown', function(e){
  ...
  /* the palette lives in js/palette.js now, so its open state is read from the DOM */
  var paletteOpen = !!document.querySelector('.cmdk-scrim.show');
  if (paletteOpen || treeScrim.classList.contains('show') || gpanel.classList.contains('open')) return;
