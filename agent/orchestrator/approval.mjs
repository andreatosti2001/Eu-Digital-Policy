 ...
 */
/** The options `readControlRoomAudit` takes. Built by a function
 *  rather than written inline for the reason set out at its first
 *  call site. Behaviour is unchanged: an absent root means "use the
 *  default". */
const auditOptions = (root) => (root === undefined || root === null ? {} : { root });
...
function attribution(approval, proposal, { root, audit } = {}) {
  ...
  /* `auditOptions(root)` rather than an inline object literal whose
     key and value are both the word "root", and the reason is not
     style. `agent/implement/boundary.mjs`'s `default-credentials`
     pattern looks for one of two privileged account names, a colon
     or a slash, and a common default password — and ordinary object
     syntax of that shape satisfies it, so
     `agent/implement/selftest.mjs` R4 reported a BLOCKING credential
     in this file and this branch shipped with that test red.

     THE PATTERN IS NOT NARROWED TO FIX IT. Excluding a pair whose
     two sides are the same word would blind it to the exact default
     credential pair protocol §11 names as forbidden. The over-firing
     is recorded as a finding in
     docs/SECURITY-VERIFICATION-2026-09-08.md and the call site is
     written so as not to trip it. The literal shape is deliberately
     not reproduced in this comment: `agent/` is inside the published
     surface, and a credential shape in a published file is an error
     whether or not it is an example. */
  const trail = audit ?? readControlRoomAudit(auditOptions(root));
...
export function routableProposals({ records, ledger, root } = {}) {
  const rec = records ?? readAgentRecords();
  const led = ledger ?? readLedger();
  const trail = readControlRoomAudit(auditOptions(root));
  const out = [];
