...
test('R4 · complete_provenance is one of the twelve mandatory conditions and fails on a blocking question', () => {
  ...
  assert.ok(a.failed.some((c) => c.condition === 'complete_provenance'));
});
...
test('R6 · no action category is approved for automatic execution, and the reason is stated', () => {
  ...
  assert.equal(a.permitted, false);
  const c = a.conditions.find((x) => x.condition === 'allowed_action_category');
...
test('R6 · a validator that did not run is not a validator that passed', () => {
  ...
  const c = a.conditions.find((x) => x.condition === 'project_validators');
  assert.equal(c.satisfied, false);
...
test('R6 · browser QA is required by SCOPE, and a missing run is not a pass', () => {
  ...
  assert.equal(notNeeded.conditions.find((c) => c.condition === 'browser_qa_where_required').satisfied, true);
  const needed = autonomyPermits({ scope: { requires_browser_qa: true, permitted: ['index.html'], refusals: [] }, browser: null });
  assert.equal(needed.conditions.find((c) => c.condition === 'browser_qa_where_required').satisfied, false);
});
