  ...
  C('validators_at_baseline', 'validators', true,
    ...
    (f) => {
      ...
      return v === 'pass' || v === 'pass_with_findings'
        ...
        : fail(`node agent/implement/cli.mjs check reports verdict ${v} against docs/CURRENT-ARCHITECTURE.md \u00a712. ${off.map((c) => `${c.name} exit ${c.exit_code}, ${c.errors} error(s) against a baseline of ${c.baseline_errors}`).join('; ')}. This is the pre-existing state CI has recorded on every run since SESSION 24, and it is a standing disagreement nobody has decided: either freshness.mjs should not exit 1 on "1 item(s) need attention", or \u00a712's recorded baseline of 0 errors for it is wrong. The item itself is real \u2014 the most recent enforcement decision on record is 49 days old \u2014 and on a production site that is content work, not a tooling defect.`);
    }),
